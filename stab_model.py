# import matplotlib
# matplotlib.use('Agg')  # 或 'Qt5Agg', 'Agg' (非交互式)
...
class Stab:
    ...
    def CalcForces(self):
        ...
        if SwModel == 0:  # 实模型
            ...
            if fh1 > 0.0:  # 与下空腔壁接触
                ...
                V2 = -DS[int(Jc1[Imin1]) + 1] / (2.0 * PI * Rcav[Jend, cav_idx]) + Vp * RibTan[Imin1]
            ...
            if fh2 > 0.0:  # 与上空腔壁接触
                ...
                # 同步修改
                V2 = DS[int(Jc2[Imin2]) + 1] / (2.0 * PI * Rcav[Jend, cav_idx]) - Vp * RibTan[Imin2]
    ...
    def TurnModelOnGamma(self, gamma_rad):
    ...
    def CalcUnsteadyCavity(self):
        ...
        for j in range(2, Jend + 2):  # Jend+1 for inclusive range
            start_idx = Jend - j + 2
            end_idx = Jend
            for i in range(start_idx, end_idx):  # +1 for inclusive range
        ...
        for i in range(2 - 1, Jend + 2 - 1):  # i = 2 to Jend inclusive
            ...
            # Store cavity radius
            if Si < 0.0:
                if Jend < Rcav_new.shape[0] and i < Rcav_new.shape[1]:
                    Rcav_new[Jend, i] = 0.0
                Ipr = i
            else:
                radius = np.sqrt(Si / PI)
                if Jend < Rcav_new.shape[0] and i < Rcav_new.shape[1]:
                    Rcav_new[Jend, i] = radius
                Ipr = i
        ...
        for i in range(2 - 1, Jend + 2 - 1):
            ...
            rcav_val = Rcav_new[Jend, i] if Jend < Rcav_new.shape[0] and i < Rcav_new.shape[1] else 0.0

            if i + 1 <= max_dim:
                Ypr2[i] = Yax0[yax0_idx] + hfi + rcav_val
                Ypr1[i] = Yax0[yax0_idx] + hfi - rcav_val
    ...
    def calculation_plot_point(self):
        if self.FlagDive:
            # === 计算旋转角度 ===
            gamma_rad = np.deg2rad(self.GammaDive)  # 入水角度转为弧度
            cos_g = np.cos(gamma_rad)
            sin_g = np.sin(gamma_rad)

            # 计算初始偏移
            xs0 = -self.Rn * cos_g ** 2 / sin_g if abs(sin_g) > 1e-10 else 0
            ys0 = self.Rn * cos_g
            self.xs0 = xs0
            self.ys0 = ys0
            self.COS_G = cos_g
            ...
            # 设置全局变量
            self.GammaRad = gamma_rad

            # 旋转模型
            self.TurnModelOnGamma(gamma_rad)

            # 应用位移
            xs2, ys2 = self.TurnPointOnGamma(self.xs1, self.Yabs)
            xs2 = xs2 - xs0
            ys2 = ys2 - ys0
            if not hasattr(self, 'xc0'):
                # 加入绘图记录初始位置，方便后面那些东东转移，可能用不到
                self.xc0, self.yc0 = self.TurnPointOnGamma(-self.Xabs, self.Yabs)

            xc, yc = self.TurnPointOnGamma(-self.Xabs, self.Yabs)
            ...
            lower_y = []
            # 主要修复：增强空腔轮廓生成
            if hasattr(self, 'Xpr') and hasattr(self, 'Ypr1') and hasattr(self, 'Ypr2') and hasattr(self, 'Ipr'):
                # 1. 上部空腔轮廓
                iup = 3
                if ys2 + self.YGmod[1] < 0:  # 上分离点在水下
                    # 起始点（上分离点）
                    cavity_points_upper = [(xs2 + self.XGmod[1], ys2 + self.YGmod[1])]

                    # 检查是否有足够的点进行处理
                    if hasattr(self, 'XcBeg') and len(self.XcBeg) >= 4:
                        # 处理前几个特殊点
                        xpr_val = self.Xpr[1]  # Fortran索引2

                        if xpr_val < self.XcBeg[1]:  # XcBeg(2)
                            RcBegX = self.Rn * (1.0 + 3.0 * xpr_val / self.Rn) ** (1.0 / 3.0)
                            xt, yt = self.TurnPointOnGamma(xpr_val, RcBegX + self.Yax0[self.Jend])
                            if ys2 + yt <= 0:
                                cavity_points_upper.append((xs2 + xt, ys2 + yt))

                        elif xpr_val < self.XcBeg[2]:  # XcBeg(3)
                            xt, yt = self.TurnPointOnGamma(self.XcBeg[1], self.RcBeg[1] + self.Yax0[self.Jend])
                            if ys2 + yt <= 0:
                                cavity_points_upper.append((xs2 + xt, ys2 + yt))
                            RcBegX = self.Rn * (1.0 + 3.0 * xpr_val / self.Rn) ** (1.0 / 3.0)
                            xt, yt = self.TurnPointOnGamma(xpr_val, RcBegX + self.Yax0[self.Jend])
                        ...
                        elif xpr_val < self.XcBeg[3]:  # XcBeg(4)
                            xt, yt = self.TurnPointOnGamma(self.XcBeg[1], self.RcBeg[1] + self.Yax0[self.Jend])
                            if ys2 + yt + self.RcBeg[1] <= 0:
                                cavity_points_upper.append((xs2 + xt, ys2 + yt))
                            xt, yt = self.TurnPointOnGamma(self.XcBeg[2], self.RcBeg[2] + self.Yax0[self.Jend])
                            if ys2 + yt <= 0:
                                cavity_points_upper.append((xs2 + xt, ys2 + yt))
                            RcBegX = self.Rn * (1.0 + 3.0 * xpr_val / self.Rn) ** (1.0 / 3.0)
                            xt, yt = self.TurnPointOnGamma(xpr_val, RcBegX + self.Yax0[self.Jend])
                            ...
                            if ys2 + yt <= 0:
                                cavity_points_upper.append((xs2 + xt, ys2 + yt))
                            xt, yt = self.TurnPointOnGamma(self.XcBeg[3], self.RcBeg[3] + self.Yax0[self.Jend])
                            if ys2 + yt <= 0:
                                cavity_points_upper.append((xs2 + xt, ys2 + yt))
                            xt, yt = self.TurnPointOnGamma(xpr_val, self.Ypr2[1])
                            if ys2 + yt <= 0:
                                cavity_points_upper.append((xs2 + xt, ys2 + yt))

                    for i in range(3, self.Ipr + 2):
                        ...
                        # 根据x的位置选择正确的y值
                        if hasattr(self, 'XcBeg') and len(self.XcBeg) >= 4:
                            if self.Xpr[idx] < self.XcBeg[1]:  # XcBeg(2)
                                RcBegX = self.Rn * (1.0 + 3.0 * self.Xpr[idx] / self.Rn) ** (1.0 / 3.0)
                                xt, yt = self.TurnPointOnGamma(self.Xpr[idx], RcBegX + self.Yax0[self.Jend])
                                if ys2 + yt <= 0:
                                    cavity_points_upper.append((xs2 + xt, ys2 + yt))

                            elif self.Xpr[idx] < self.XcBeg[2]:  # XcBeg(3)
                                RcBegX = self.Rn * (1.0 + 3.0 * self.Xpr[idx] / self.Rn) ** (1.0 / 3.0)
                                xt, yt = self.TurnPointOnGamma(x, RcBegX + self.Yax0[self.Jend])
                                if ys2 + yt <= 0:
                                    cavity_points_upper.append((xs2 + xt, ys2 + yt))

                            elif self.Xpr[idx] < self.XcBeg[3]:  # XcBeg(4)
                                RcBegX = self.Rn * (1.0 + 3.0 * self.Xpr[idx] / self.Rn) ** (1.0 / 3.0)
                                xt, yt = self.TurnPointOnGamma(x, RcBegX + self.Yax0[self.Jend])
                                if ys2 + yt <= 0:
                                    cavity_points_upper.append((xs2 + xt, ys2 + yt))

                            elif self.Rcav[self.Jend, idx] != 0:
                                xt, yt = self.TurnPointOnGamma(self.Xpr[idx - 1], self.Ypr2[idx - 1])
                                if ys2 + yt <= 0:
                                    cavity_points_upper.append((xs2 + xt, ys2 + yt))
                                xt, yt = self.TurnPointOnGamma(self.Xpr[idx], self.Ypr2[idx])
                                if ys2 + yt <= 0:
                                    cavity_points_upper.append((xs2 + xt, ys2 + yt))
                            else:
                                a = 1
                    ...
                    if self.Ipr > 2:
                        ...
                        while True:
                            x = x + 0.001
                            y = self.ExtraUpperCav(x)
                            xt, yt = self.TurnPointOnGamma(x, y)
                            if ys2 + yt >= 0:
                                cavity_points_upper.append((xs2 + xt, ys2 + yt))
                                break
                    ...
                    if len(cavity_points_upper) >= 1:
                        # 再次去重并排序
                        cavity_points_upper = self.remove_duplicate_points(cavity_points_upper)
                ...
                # 5. 修复下部空腔轮廓
                lower_sep_idx = 2 * self.Ncon + 3 - 1  # Fortran索引 2*Ncon+3 -> Python索引 2*Ncon+2
                if lower_sep_idx < len(self.XGmod) and lower_sep_idx < len(self.YGmod):
                    cavity_points_lower = [(xs2 + self.XGmod[lower_sep_idx], ys2 + self.YGmod[lower_sep_idx])]
                    ...
                    if hasattr(self, 'XcBeg') and len(self.XcBeg) >= 4:
                        ...
                        if xpr_val < self.XcBeg[1]:  # XcBeg(2)
                            ...
                            xt, yt = self.TurnPointOnGamma(self.XcBeg[3], -self.RcBeg[3] + self.Yax0[self.Jend])
                            cavity_points_lower.append((xs2 + xt, ys2 + yt))
                            xt, yt = self.TurnPointOnGamma(xpr_val, self.Ypr1[1])
                            cavity_points_lower.append((xs2 + xt, ys2 + yt))
                    ...
                    for i in range(3, self.Ipr + 2):
                        ...
                        if hasattr(self, 'XcBeg'):
                            if x < self.XcBeg[1]:  # XcBeg(2)
                                RcBegX = self.Rn * (1.0 + 3.0 * x / self.Rn) ** (1.0 / 3.0)
                                xt, yt = self.TurnPointOnGamma(x, -RcBegX + self.Yax0[self.Jend])
                                cavity_points_lower.append((xs2 + xt, ys2 + yt))

                            elif x < self.XcBeg[2]:  # XcBeg(3)
                                RcBegX = self.Rn * (1.0 + 3.0 * x / self.Rn) ** (1.0 / 3.0)
                                xt, yt = self.TurnPointOnGamma(x, -RcBegX + self.Yax0[self.Jend])
                                cavity_points_lower.append((xs2 + xt, ys2 + yt))

                            elif x < self.XcBeg[3]:  # XcBeg(4)
                                RcBegX = self.Rn * (1.0 + 3.0 * x / self.Rn) ** (1.0 / 3.0)
                                xt, yt = self.TurnPointOnGamma(x, -RcBegX + self.Yax0[self.Jend])
                                cavity_points_lower.append((xs2 + xt, ys2 + yt))

                            elif self.Rcav[self.Jend, idx] != 0:
                                xt, yt = self.TurnPointOnGamma(self.Xpr[idx - 1], self.Ypr1[idx - 1])
                                cavity_points_lower.append((xs2 + xt, ys2 + yt))
                                xt, yt = self.TurnPointOnGamma(self.Xpr[idx], self.Ypr1[idx])
                                cavity_points_lower.append((xs2 + xt, ys2 + yt))
                    ...
                    if len(cavity_points_lower) >= 1:
                        # 去重并排序
                        cavity_points_lower = self.remove_duplicate_points(cavity_points_lower)
            ...
            return upper_x, upper_y, lower_x, lower_y, model_points[:, 0], model_points[:, 1]
        else:
            ...
            # === 绘制空腔轮廓 ===
            if hasattr(self, 'FlagCav') and self.FlagCav:
            ...
            return upper_x, upper_y, lower_x, lower_y, self.Xmod, self.Ymod
    ...
    def Dynamics(self):
        ...
        # ========== Step 3: 初始化计算参数 ==========
        if self.FlagPath:
            self.Wwidth = 2.4 * self.Scale

        elif self.FlagDive:
            self.Wwidth = 1 * self.Scale
        else:
            aa = 1
        ...
        # ========== Step 7: 主循环 ==========
        Ndist = int(self.gXfin / self.HX) + 1
        j = 0
        last_callback_time = 0
        min_callback_interval = 0.05
        for j in tqdm(range(1, Ndist + 1)):  # j = 1 to Ndist (0-based index for step j)
            Xn = j * HX
            self.Xn = Xn
            ...
            if self.FlagDive:
                self.xs1 = -Xn
            if self.FlagPath:
                self.xs1 = self.gXfilm + self.Wwidth - Xn
            ...
            if j < Nview:
                ...
                self.Ypr1[:Nview - 1] = self.Ypr1[1:Nview]
                self.Ypr2[:Nview - 1] = self.Ypr2[1:Nview]
                ## 继续添加
            # 进行gamma的sin，con的更新
            self.SIN_G = np.sin(self.GammaRad)
            self.COS_G = np.cos(self.GammaRad)
            ...
            # 计算空泡
            upper_x, upper_y, lower_x, lower_y, model_x, model_y = self.calculation_plot_point()
            # ======================== 插入绘制图像在这里！ ============================
            # plt.figure()

            # self.plot_dynamics(show_plot=False, save_plot=True,
            #                    plot_filename=f"./fig/dynamics_step_{str(j).rjust(6, '0')}.png")
            # self.plot_dive(show_plot=False, save_plot=True,
            #                plot_filename=f"./fig/dynamics_step_{str(j).rjust(6, '0')}.png")

            # self.plot_dive_changed(show_plot=False, save_plot=True,
            #                plot_filename=f"./fig/dynamics_step_{str(j).rjust(6, '0')}.png")
            ...
            if hasattr(self, 'update_callback') and current_time - last_callback_time > min_callback_interval:
                ...
                if self.update_callback:
                    ...
                    data = {
                        ...
                        'points': {
                            # upper_x, upper_y, lower_x, lower_y, model_x, model_y
                            'upper_x': upper_x,
                            'upper_y': upper_y,
                            'lower_x': lower_x,
                            'lower_y': lower_y,
    ...
    # 辅助函数：移除重复或接近的点
    def remove_duplicate_points(self, points, tolerance=1e-5):
        if len(points) <= 1:
            return points
    ...
    def plot_dive(self, show_plot=False, save_plot=False, plot_filename="dive_plot.png"):
        """
        绘制模型入水过程的模拟图形，进一步修复空腔轮廓连接问题

        修复要点：
        1. 增强前几步的外推逻辑，确保连接到水面
        2. 优化点序列去重，消除冗余线条
        3. 精确计算轮廓与水面的交点
        4. 改进过渡点处理，确保平滑连接
        5. 增加调试信息，便于追踪问题
        ...
        hwidth = self.Hwidth
        # 水下区域
        water_polygon = plt.Polygon([[-hwidth, 0], [hwidth, 0], [hwidth, -hwidth], [-hwidth, -hwidth]],
                                    color='lightblue', alpha=0.7)
        ax.add_patch(water_polygon)

        # 水上区域
        above_water_polygon = plt.Polygon([[-hwidth, 0], [hwidth, 0], [hwidth, hwidth], [-hwidth, hwidth]],
        ...
        # === 绘制加强肋 ===
        if hasattr(self, 'XGrib') and hasattr(self, 'YGrib'):
            for j in range(0, self.Ncon - 1):
                idx1 = 2 * j
                idx2 = 2 * j + 1
                if idx1 < len(self.XGrib) and idx2 < len(self.XGrib):
                    plt.plot([xs2 + self.XGrib[idx1], xs2 + self.XGrib[idx2]],
                             [ys2 + self.YGrib[idx1], ys2 + self.YGrib[idx2]],
                             'k-', linewidth=1)
        ...
        # === 绘制水面和参考线 ===
        plt.plot([-hwidth, hwidth], [0, 0], 'b-', linewidth=1.5, label='水面')
        ...
        # === 添加文本标注和参数显示 ===
        plt.title("FILMING THE WATER ENTRY", fontsize=14, fontweight='bold')

        # 参数显示
        param_text = (
            f"CONTACT PARAMETERS:\n"
            f"Dn = {self.Dnmm:.2f} mm\n"
            f"δ = {self.Delta:.2f}°\n"
            f"γ = {self.GammaDive:.2f}°\n"
            f"V0 = {self.V0:.2f} m/s\n\n"
            f"CURRENT PARAMETERS:\n"
            f"t = {self.Tabs * self.Lm * 1000 / self.V0:.4f} ms\n"
            f"x = {(self.Xabs + self.Xc) * self.Lm:.4f} m\n"
            f"y = {self.Yabs * self.Lm * 1000:.4f} mm\n"
            f"Vx/V0 = {self.VxAbs:.4f}\n"
            f"ψ = {self.Psi:.4f} rad\n"
            f"α = {self.AlphaCur:.4f} rad\n"
            f"ω = {self.Omega * self.V0 / self.Lm:.4f} rad/s"
        )

        plt.figtext(0.75, 0.65, param_text, fontsize=9,
                    bbox=dict(facecolor='white', alpha=0.8, edgecolor='gray'))

        # 力系数
        if hasattr(self, 'Cnx') and hasattr(self, 'Cny') and hasattr(self, 'Cnm'):
            forces_text = (
            ...
            plt.figtext(0.75, 0.40, forces_text, fontsize=9,
                        bbox=dict(facecolor='white', alpha=0.8, edgecolor='gray'))
        ...
        if hasattr(self, 'SGmax') and hasattr(self, 'TAUmax'):
            stress_text = (
                f"MAXIMAL STRESSES:\n"
                f"σmax = {self.SGmax:.2f} MPa\n"
                f"τmax = {self.TAUmax:.2f} MPa"
            )
            plt.figtext(0.75, 0.30, stress_text, fontsize=9,
                        bbox=dict(facecolor='white', alpha=0.8, edgecolor='gray'))

        # 模式信息
        mode_text = "NON-STOP mode" if self.SwDisp == 0 else "STEP-BY-STEP mode"
        plt.figtext(0.1, 0.02, f"This is {mode_text}.", fontsize=9)

        step_info = f"Press any key for next step. HX/L = {self.HXdive:.3f}"
        plt.figtext(0.6, 0.02, step_info, fontsize=9)
        ...
        # 设置刻度
        y_ticks = np.linspace(-hwidth, hwidth, 5)
        x_ticks = np.linspace(-hwidth, hwidth, 5)
        ...
        plt.figtext(0.45, 0.05, scale_text, fontsize=9, bbox=dict(facecolor='white', alpha=0.8))

        # 添加图例
        plt.figtext(0.05, 0.95, "- mass center", fontsize=9, color='red')
    ...
    def plot_dive_changed(self, show_plot=False, save_plot=False, plot_filename="dive_plot.png"):
        """
        绘制模型入水过程的模拟图形，进一步修复空腔轮廓连接问题
        ...
        if hasattr(self, 'XGrib') and hasattr(self, 'YGrib'):
            for j in range(0, self.Ncon - 1):
                ...
                idx2 = 2 * j + 1
                if idx1 < len(self.XGrib) and idx2 < len(self.XGrib):
                    plt.plot([xs2 + self.XGrib[idx1] - xc, xs2 + self.XGrib[idx2] - xc],
                             [ys2 + self.YGrib[idx1] - yc, ys2 + self.YGrib[idx2] - yc],
                             'k-', linewidth=1)
